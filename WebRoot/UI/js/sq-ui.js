(function($) {
	...
	function setHeightDiv(id) {
		var wcc = $("#wc_" + id).children();
		var wcch = $("#wc_" + id).height();
		var dos = {};
		var dnum = 0;
		for (var i = 0; i < wcc.length; ++i) {
			if (!wcc[i].ptid) {
				wcch -= $(wcc[i]).height() + 22
			} else {
				var _wid = wcc[i].id;
				if (_wid.indexOf("p_") > -1) {
					dos[_wid] = wcc[i].ptid;
					wcch -= 15;
					++dnum
				}
	...
	var createWinPannel = function(id, ww, wh, opt) {
		...
		for (var i = 0; i < optArr.length; ++i) {
			var fo = createContent(id, optArr[i], yuHeight, cww);
			if (fo) {
				wc.appendChild(fo)
			}
		}
	...
	var createContent = function(id, opt, h, w) {
		var fo = null;
		if (opt.type == "form") {
			var to = $.newEl("table");
			to.id = "wct_" + id;
			...
			if (opt.type == "pannel") {
				...
				if (opt.height) {
					fso.offsetHeight = opt.height + "px"
				}
				if (opt.field) {
					var lo = $.newEl("legend");
				...
				divo.style.width = (w - 10) + "px";
				fso.appendChild(divo);
				fso.ptid = opt.id;
				$.addCls(fso, "pannel", "p_" + opt.id);
				fo = fso
			}
		}
		return fo
				...
				if (item.region == "center") {
					...
					if (item.region == "right") {
						if (lyMap["left"].w != 0 && lyMap["center"].w != 0) {
							pl = 4 + 3 * mag + lyMap["left"].w
									+ lyMap["center"].w
						} else {
							if (lyMap["center"].w == 0 && lyMap["left"].w == 0) {
								pl = mag
							} else {
								pl = 2 + 2 * mag + lyMap["left"].w
										+ lyMap["center"].w
...
$.extend({
				...
				$.ajax({
					...
					error : function(XMLHttpRequest, textStatus, errorThrown) {
						if (XMLHttpRequest.status != "200") {
							alert("后台异常->状态码:" + XMLHttpRequest.status
									+ "; 请求完毕状态码:" + XMLHttpRequest.readyState
									+ "; 文本状态:" + textStatus + "\n" + "异常:"
									+ errorThrown)
						}
					}
				...
				if (obj.url) {
					...
					if (obj.option && obj.option.length > 0) {
						for (var i = 0; i < obj.option.length; ++i) {
							var op = new Option(obj.option[i].text,
									obj.option[i].value);
							ino.options.add(op)
						}
					}
			...
			loadFormDate : function(id, url, para) {
				$.ajax({
					...
					error : function(XMLHttpRequest, textStatus, errorThrown) {
						alert("后台异常:" + XMLHttpRequest.status + "|"
			...
			createTip : function(t, tn) {
				...
				if (tn) {
					$(document.body).append(
							"<div class='ui-tooltip' id='" + tid
									+ "' style='top:" + t + "px;left:" + l
									+ "px;'>" + "<span class='tip-word'>" + tn
									+ "</span></div>");
					to.get(0).ttid = tid
				}
			},
			...
			onFormValidator : function(id) {
				...
				$("#" + id).delegate("input,textarea,select", "focus",
						function() {
							$.rmValiTn(this);
							$.rmTip(this)
						});
				$("#" + id).delegate("input,textarea,select", "mouseenter",
						function() {
							if (this.vd && this.className == "verify") {
								$.createTip(this, this.ttn)
							}
						});
				$("#" + id).delegate("input,textarea,select", "mouseleave",
						function() {
							if (this.vd) {
								$.rmTip(this)
							}
			...
			isIP : function(ip) {
				...
				if (reSpaceCheck.test(ip)) {
					...
					if (RegExp.$1 <= 255 && RegExp.$1 >= 0 && RegExp.$2 <= 255
							&& RegExp.$2 >= 0 && RegExp.$3 <= 255
							&& RegExp.$3 >= 0 && RegExp.$4 <= 255
							&& RegExp.$4 >= 0) {
						return true
					} else {
						return false
			...
			if (item == "-") {
				...
				if (!item.type || item.type == "button") {
					if (!item.code) {
						...
						if (item.code && isHasBtn(parr, item.code)) {
							bo = createBtn(item);
							item.id = bo.id;
							f = true;
							if (item.on) {
