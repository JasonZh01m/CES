	...
	button:[
	    ...
	    {name:"导出Excel", type:"button", icon:cp+"/UI/css/icon/folder_up.png", on:function(){
	    	...
	    	if(getCookie("userId")!=undefined || getCookie("userId")!='') {
		    	...
		    	var url = cp+"/report/satisfactionDescByExcelJson.do?startTime="+startTime+"&endTime="+endTime+"&orgCode="+orgCode
		    	window.open(url, "员工满意度排名报表");
	    	} else {
...
$("#mainGrid").jqGrid({
	...
	colNames:['机构名','员工姓名','非常满意次数','一般满意次数','不满意次数','平均满意度','业务量'], 
	colModel:[
