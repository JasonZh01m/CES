	...
	button:[
	    ...
	    {name:"导出Excel", type:"button", icon:cp+"/UI/css/icon/folder_up.png", on:function(){
	    	if(getCookie("userId")!=undefined || getCookie("userId")!='') {
		    	...
		    	var url = cp+"/report/satisfactionByExcelJson.do?startTime="+startTime+"&endTime="+endTime+"&orgCode="+orgCode
		    	window.open(url, "员工满意度报表");
	    	} else {
...
$("#mainGrid").jqGrid({
	...
	height:h-53-sbh,
	mtype: 'POST',
	url:cp+'/report/satisfactionJson.do', 
	datatype: "json", 
	altclass:'odd',
	jsonReader:{
	    root:'rows',
	...
	colNames:['机构名','员工姓名','平均满意度占比','非常满意占比','一般满意占比','不满意占比','业务量'], 
	colModel:[
		{name:'orgName',index:'orgName', width:80,search: false,hidden:false,sortable:false},
		{name:'empName',index:'empName', width:80,hidden:false,sortable:false},
