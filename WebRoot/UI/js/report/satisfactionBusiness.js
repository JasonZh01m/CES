	...
	button:[
	    ...
	    {name:"导出Excel", type:"button", icon:cp+"/UI/css/icon/folder_up.png", on:function(){
	    	if(getCookie("userId")!=undefined || getCookie("userId")!='') {
		    	...
		    	var url = cp+"/report/businessByExcelJson.do?startTime="+startTime+"&endTime="+endTime+"&orgCode="+orgCode
		    	window.open(url, "业务满意度报表");
	    	} else {
...
$("#mainGrid").jqGrid({
	...
	height:h-53-sbh,
	mtype: 'POST',
	url:cp+'/report/satisfactionByBusinessJson.do', 
	datatype: "json", 
	altclass:'odd',
	jsonReader:{
	    root:'rows',
	...
	colNames:['业务名称','平均满意度占比','非常满意占比','一般满意占比','不满意占比','业务量'], 
	colModel:[
		{name:'serviceName',index:'serviceName', width:80,search: false,hidden:false,sortable:false},
		{name:'agvsSatis',index:'agvSatis', width:80,hidden:false,sortable:false}, 
		{name:'agvPoint1',index:'agvPoint1', width:80,hidden:false,sortable:false}, 
		{name:'agvPoint3',index:'agvPoint3', width:80,hidden:false,sortable:false}, 
		{name:'agvPoint5',index:'agvPoint5', width:80,hidden:false,sortable:false}, 
