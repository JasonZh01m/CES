	...
	button:[
	    ...
	    {name:"导出Excel", type:"button", icon:cp+"/UI/css/icon/folder_up.png", on:function(){
	    	if(getCookie("userId")!=undefined || getCookie("userId")!='') {
		    	...
		    	var url = cp+"/report/businessDescByExcelJson.do?startTime="+startTime+"&endTime="+endTime+"&orgCode="+orgCode
		    	window.open(url, "业务满意排行报表");
	    	} else {
	    		alert("用户超时，请点击退出系统重新登录再做操作！");
	    		return;
	    	}
//	    	window.open(cp+"/report/businessDescByExcelJson.do", "业务满意排行报表", param);
	    }}
	]
});
...
$("#mainGrid").jqGrid({
	...
	colNames:['业务名','非常满意次数','一般满意次数','不满意次数','平均满意度','业务量'], 
	colModel:[
