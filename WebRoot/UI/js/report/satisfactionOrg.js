	...
	button:[
	    ...
	    {name:"导出Excel", type:"button", icon:cp+"/UI/css/icon/folder_up.png", on:function(){
	    	if(getCookie("userId")!=undefined || getCookie("userId")!='') {
		    	...
		    	var url = cp+"/report/satisOrgByExcelJson.do?startTime="+startTime+"&endTime="+endTime+"&orgCode="+orgCode
		    	window.open(url, "机构满意度报表");
	    	} else {
...
$("#mainGrid").jqGrid({
	...
	colNames:['机构名','平均满意度占比','非常满意占比','一般满意占比','不满意占比','业务量'], 
	colModel:[
