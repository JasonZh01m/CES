...
public class CacheAspect {
	...
	private final String sqlCacheStr = "sqlCache";
	...
	private String getClsName(Object dao) {
		return this.daoMap.get(dao.getClass().getSimpleName());
	}
	...
	public Object getModelInCacheByCode(ProceedingJoinPoint pjp) {
		...
		try {
			...
			ModelCacheInfo mci = this.map.get(clsName);
			if (mci != null) {
			...
			if ((mci != null) && (rs > 0)) {
				...
				if ((relationCls != null) && (relationCls.size() > 0)) {
					for (int i = 0; i < relationCls.size(); i++) {
						...
						ReferenceClassInfo rci = this.refMap
								.get(relationClsName);
	...
	public Object insertInCache(ProceedingJoinPoint pjp) {
		...
		try {
			...
			if (pjp.getArgs().length == 2) {
				...
				if ((retVal != null)
						&& (this.intStr.equals(retVal.getClass()
								.getSimpleName()))) {
				...
				if ((mci != null) && (id.intValue() > 0)) {
					String key = AnnotationInfoUtil
							.getCacheKeyForModel(mci, id);
					clearSqlCache(dao);
					this.cacheUtil.put(key, o);
				}
				ReferenceClassInfo rci = this.refMap.get(clsName);
				if (rci != null) {
					...
					if (rkm.isSingleKey()) {
						...
						if (rvs.length == 2) {
							String key1 = rvs[0].getFkName() + "_"
									+ rvs[0].getValue();
							String key2 = rvs[1].getFkName() + "_"
									+ rvs[1].getValue();
							if (rc.get(key1) != null) {
								rc.putById(key1, key);
							}
			...
			if (("DuplicateKeyException".intern().equals(expName))
					|| ("MySQLIntegrityConstraintViolationException".intern()
							.equals(expName))) {
	...
	private Object newInstanceObj(Class cls, String id,
			List<ReferenceInfo> refSet) {
		Object o = null;
		System.out.println();
		...
		try {
			...
			for (int i = 0; i < refSet.size(); i++) {
				ReferenceInfo rfi = refSet.get(i);
	...
	private void clearSqlCache(Object dao) {
		try {
			SqlCacheForClass sqlCache = (SqlCacheForClass) ClassUtil
					.getFieldValue(dao, this.sqlCacheStr);
			sqlCache.clear();
		} catch (Exception e) {
			e.printStackTrace();
		}
